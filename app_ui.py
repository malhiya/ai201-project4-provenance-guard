    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data['attribution'] == "likely_human":
                # If score is 0.0 -> 100% human. If score is 0.40 -> 60% human.
                display_pct = round((1.0 - raw_confidence) * 100)
                
            elif data['attribution'] == "likely_ai":
                # If score is 1.0 -> 100% AI. If score is 0.80 -> 80% AI.
                display_pct = round(raw_confidence * 100)
                
            else:
                # Uncertain: confidence reflects distance from the 0.5 midpoint, so a
                # dead-center score (maximally uncertain) reads as low confidence, not high.
                display_pct = round(abs(raw_confidence - 0.5) * 2 * 100)

            # Ensure the percentage never drops below 0% or above 100% due to minor rounding
